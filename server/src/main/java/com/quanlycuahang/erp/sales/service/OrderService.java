...
public class OrderService {
  ...
  public OrderResponse createOrder(OrderCreateRequest request, String idempotencyKey) {
    ...
    if (grandTotal.compareTo(request.getExpectedTotalAmount()) != 0) {
      throw new BusinessRuleException(
  ...
  private String generateOrderNumber() {
    ...
    long sequence =
        numberSequenceService.nextValue(TenantContext.get(), NumberSequenceService.ORDER_NUMBER_SEQ);
  ...
  private String generateInvoiceNumber() {
    ...
    long sequence =
        numberSequenceService.nextValue(
            TenantContext.get(), NumberSequenceService.INVOICE_NUMBER_SEQ);
    return prefix + String.format("%06d", sequence);
  }
}
