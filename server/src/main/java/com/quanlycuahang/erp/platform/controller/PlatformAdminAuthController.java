...
import com.quanlycuahang.erp.platform.service.PlatformAdminAuthService;
...
/**
 * Dang nhap rieng cho Super Admin - path/cookie TACH BIET hoan toan khoi /api/v1/auth cua tenant
 * User (xem PlatformAdminAuthService, PlatformAdminJwtAuthenticationFilter).
 */
@RestController
...
public class PlatformAdminAuthController {
  ...
  private static final String REFRESH_COOKIE_PATH = "/api/v1/platform-admin/auth";
  ...
  public ResponseEntity<ApiResponse<AccessTokenResponse>> login(
    ...
    AuthTokens tokens =
        authService.login(
            request.getUsername(), request.getPassword(), ClientIpResolver.resolve(httpRequest));
