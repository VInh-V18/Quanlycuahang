...
public interface DebtRepository extends JpaRepository<Debt, Long> {
  ...
  @Query(
      ...
      nativeQuery = true)
  List<Object[]> findAgingByPartner(@Param("direction") String direction);
  ...
  @Query(
      value =
          "SELECT * FROM ("
              ...
              + "FROM debts d "
              + "WHERE d.direction = :direction AND d.deleted_at IS NULL "
              ...
              + "WHERE d.direction = :direction AND dp.deleted_at IS NULL "
              + "AND (:customerId IS NULL OR d.customer_id = CAST(:customerId AS bigint)) "
              + "AND (:supplierId IS NULL OR d.supplier_id = CAST(:supplierId AS bigint))"
