...
const PurchaseOrderDetailPage = lazy(() =>
  import("@/pages/purchase-orders/PurchaseOrderDetailPage").then((m) => ({
...
const PlatformAdminPage = lazy(() =>
  import("@/pages/platform-admin/PlatformAdminPage").then((m) => ({
...
const router = createBrowserRouter([
  ...
  {
    // Trang Super Admin - tach biet hoan toan khoi RequireAuth/MainLayout cua tenant User (dang
    // nhap/quyen rieng, xem PlatformAdminPage.tsx).
    path: "/platform-admin",
    element: withSuspense(<PlatformAdminPage />),
  },
  {
    element: (
      <RequireAuth>
    ...
    children: [
      ...
      },
      {
        path: "/returns",
        element: withSuspense(
          <RequirePermission perm="return:view">
            <ReturnsSearchPage />
          </RequirePermission>,
        ),
      },
